...
[AddComponentMenu("")]
[ExecuteInEditMode]
public class GAFMaskedBehaviour : GAFAnimationBehaviour 
{
	#region Members
	
	[HideInInspector] [System.NonSerialized] private GAFMaskBehaviour m_Mask = null;

	#endregion // Members

	#region Interface

	public virtual void UpdateMask(GAFMaskBehaviour _Mask)
	{
		m_Mask = _Mask;
	}
	...
	private void ApplyMask()
	{
		    ...
		    m_Mask.currentState.Alpha > 0)
		{
			...
			float scale = movieClip.settings.pixelsPerUnit * m_Mask.element.Scale * movieClip.settings.csf;
			...
			float pivotX = (float) m_Mask.element.PivotX 				/ maskWidth;
			float pivotY = (float)(maskHeight - m_Mask.element.PivotY) 	/ maskHeight;
		...
		{
			renderer.sharedMaterial.SetMatrix("_TransformMatrix", Matrix4x4.identity);
